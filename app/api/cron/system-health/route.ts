...
async function checkOsirisPricing(): Promise<HealthCheck> {
  try {
    ...
    // House cleaning tenants need pricing tiers
    const houseCleaningTenants = tenants.filter(
      (t) => !t.service_description || t.service_description.toLowerCase().includes('cleaning')
    )
    // Exclude window cleaning (WinBros uses HCP pricing, not tiers)
    const tieredTenants = houseCleaningTenants.filter(
      (t) => !t.service_description?.toLowerCase().includes('window')
    )
