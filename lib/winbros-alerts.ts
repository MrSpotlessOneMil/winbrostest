...
export async function checkUnderfillDays(
  ...
  const { data: jobs, error } = await getSupabaseServiceClient()
    .from('jobs')
    .select('date')
    .eq('brand', 'winbros')
    .gte('date', startDate)
...
export async function checkRainDayAlerts(
  ...
  try {
    ...
    // Get affected jobs
    const { data: jobs } = await getSupabaseServiceClient()
      .from('jobs')
    ...
    await getSupabaseServiceClient().from('rain_days').upsert({
      brand: 'winbros',
      date,
      is_rain_day: true,
