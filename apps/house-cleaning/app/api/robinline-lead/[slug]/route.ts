 ...
 *
 * REQUIRED ENV (set in the osiris-house-cleaning Vercel project):
 *   ROBINLINE_PUBLIC_BASE_URL  e.g. https://app.robinline.com  (no trailing slash needed)
 */
export async function POST(
  ...
  // Mirrors the source='ci-smoke' tagging used by the legacy website webhook.
  ...
  const base = (process.env.ROBINLINE_PUBLIC_BASE_URL || "").replace(/\/+$/, "")
  if (!base) {
    console.error("[robinline-lead] ROBINLINE_PUBLIC_BASE_URL is not set")
    return NextResponse.json(
      { success: false, error: "Lead routing is not configured. Please call us." },
      { status: 500 }
    )
  }
