...
/**
 * /crew-assignment — admin daily mapping of salesman → team_lead → 2 techs.
 *
 * Restored 2026-04-29 (Phase K, Blake call addendum). Original implementation
 * lived at /crews until Round-2-Wave-2 retired that route in favor of the
 * /appointments sales grid. Blake later asked for the assignment tab back:
 * each salesman owns one crew, so the daily TL+tech roster has to live
 * somewhere admin can drag-drop it.
 *
 * Backed by `crew_days` + `crew_day_members` (role: team_lead | technician
 * | salesman) and the existing `/api/actions/crews` GET/POST handlers.
 */
