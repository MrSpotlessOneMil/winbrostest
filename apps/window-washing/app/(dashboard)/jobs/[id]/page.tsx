...
/**
 * Job/Visit Detail Page — TWO-COLUMN layout
 *
 * The core execution page for WinBros. Opened when clicking a job from the calendar.
 *
 * LEFT COLUMN:
 *   - Price / Services (line items with subtotal, discount, total)
 *   - Checklist (blocks job closure if incomplete)
 *   - Service Plan (if customer has an active membership)
 *   - Upsells (add service button visible only when in_progress)
 *
 * RIGHT COLUMN:
 *   - Visit Flow Bar (7-step sequential action bar)
 *   - Customer Info card
 *   - Job Metadata
 *   - Visit Notes (editable)
 *   - Client History (past visits)
 */
...
import {
  ArrowLeft, DollarSign, MapPin, Phone, User, Calendar,
...
export default function JobDetailPage() {
  ...
  // Notes (job notes — read-only display from job record)
  ...
  const handleCollectPayment = useCallback(() => {
    if (!data) return
    // For now, record as card payment with the job price
  ...
  const handleToggleChecklist = useCallback(async (itemId: number, completed: boolean) => {
    ...
    // If it failed, refetch to correct state
    if (!res.ok) {
      await fetchData()
    }
  }, [data, fetchData])
  ...
  }, [data, upsellName, upsellPrice, fetchData])
  ...
  if (error || !data) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <Button variant="outline" onClick={() => router.push('/jobs')} className="cursor-pointer">
  ...
  return (
    <div className="p-4 lg:p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        <Button variant="ghost" size="sm" onClick={() => router.push('/jobs')} className="cursor-pointer">
          <ArrowLeft className="w-4 h-4" />
        </Button>
        <div className="flex-1">
          <h1 className="text-xl font-bold text-white">
            {job.service_type || 'Job'} — {customerFullName}
          </h1>
          <p className="text-sm text-zinc-400">
            Job #{job.id} {job.date ? `| ${formatDate(job.date)}` : ''}
          </p>
        </div>
        <Badge className={`${statusBadgeColor(visit.status)} text-xs`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* ======== RIGHT COLUMN — Visit Flow + Customer Info ======== */}
        <div className="space-y-6">
