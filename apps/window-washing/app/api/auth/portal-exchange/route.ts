 ...
 *  - Token comparison is constant-time-ish via DB equality (Postgres handles
 *    fixed-length token comparisons).
 */
...
function isSafeNextPath(next: string): boolean {
  ...
  if (next.startsWith('//')) return false
  // Reject any next that smells like a fully-qualified URL
