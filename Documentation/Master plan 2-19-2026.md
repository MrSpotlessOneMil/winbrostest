...
## RLS Security Bypass — RESOLVED 2-19-2026

**Problem (was):**
`SUPABASE_SERVICE_ROLE_KEY` was used everywhere, bypassing Row-Level Security.

**Fix Applied:**
- Introduced `getTenantScopedClient()` using `SUPABASE_ANON_KEY` + custom HS256 JWT with `tenant_id`
- 8 tenant-scoped API routes refactored (`customers`, `jobs`, `leads`, `calendar`, `calls`, `teams`, `manage-teams`, `actions/send-sms`)
- RLS policies created on 16 tables via `scripts/05-rls-policies.sql`
- JWT created via `jose` library with 1-hour expiry

**Verification:**
- RESTRICTIVE policy test returned 0 customers for tenant user, confirming RLS enforcement
- Admin user correctly bypasses via `getSupabaseServiceClient()` (by design)
- Webhook and cron routes correctly use service role (by design)

**Full details:** `verification RLS 2-19-2026.md`

## Demo Seed Endpoint — RESOLVED 2-18-2026

**Problem (was):**
`/api/demo/seed` had no authentication — anyone could insert fake data into production.

**Fix Applied:**
- Server-side `requireAdmin()` guard as first line of POST handler
- UI "Demonstration" tab hidden for non-admin users
- Admin routes (`tenants`, `users`) consolidated to shared `requireAdmin` utility

**Verification:**
- Live attack simulation confirmed: unauthenticated POST returns 401
- Before/after comparison documented

**Full details:** `verification demo seed auth 2-18-2026.md`
